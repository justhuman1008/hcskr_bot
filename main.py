...
async def 도움말(ctx):
    ...
    Help_Embed.add_field(name="­", value=f"­", inline=True)
    Help_Embed.add_field(name="급식 조회하기", value=f":small_blue_diamond: /급식", inline=False)
...
async def 정보(ctx):
    try:
        users = json.load(open(DB_path,encoding="utf_8"))['users']
    except:
        users = "Erorr"
    ...
    bot_info.add_field(name="지원 서버", value="[서포트 서버](https://discord.gg/PS4UttDmPQ)", inline=True)
    bot_info.add_field(name="라이브러리", value="[Pycord](https://docs.pycord.dev/en/master/)", inline=True)
...
async def db다운(ctx):
    await ctx.respond(file=discord.File(DB_path))
...
def is_register(DiscordID):
    """
    유저가 자신의 정보를 입력했는지 확인하는 함수\n
    가입되어있다면 True, 아니라면 False를 반환
    """
...
def add_info(Nickname, DiscordID, Name, Birthday, Area, School, School_lv, Password):
    """
    DB에 유저의 정보를 입력하는 함수\n
    입력하는 순서는 닉네임, 아이디, 본명, 생년월일(6자), 지역명, 학교명, 학교분류, 비밀번호
    """
    register = is_register(DiscordID) # 가입되어 있을시 중지
    if register:
        return
...
async def 자가진단(ctx, 작업:Option(str,"다음 중 하나를 선택하세요.", choices=["실행", "예약", "등록", "삭제"])):
    ...
    if 작업 == "삭제":
        ...
        Delete.add_field(name="진단정보 제거시 `자가진단 매크로, 급식조회가 불가능`합니다.", value="자가진단 정보를 제거하시려면 `1분`내로 ✅를 클릭해주세요.", inline=False)
        ...
        if TrueFalse:
            class Button(discord.ui.View):
                ...
                async def on_timeout(self):
                        await Question.edit_original_message(view=None)
    ...
    if 작업 == "등록":
        if TrueFalse:
            ...
            Register_Already.set_thumbnail(url=ImageDict["List_Done"])
            ...
            class infoQ(Modal):
                ...
                async def callback(self, interaction:discord.Interaction):
                    ...
                    Register_Failed.set_thumbnail(url=ImageDict["List_Failed"])

                    reg_ntc1 = "❗ 자가진단 정보를 확인하기 위해 `1회 자가진단이 진행됩니다.`\n\n"
                    reg_ntc2 = "이후 `제공하신 정보가 저장`되며\n 유저가 `직접 삭제하기 전까지` 서비스 제공을 위해 사용되며\n\n"
                    reg_ntc3 = "`제3자에게 제공되지 않습니다.`\n\n 보유 목적: `서비스에 대한 공지`, `자가진단 서비스 진행`\n\n"
                    reg_ntc4 = "자가진단 정보를 입력하시려면 `1분`내로 ✅를 클릭해주세요."
                    reg_ntc = reg_ntc1+reg_ntc2+reg_ntc3+reg_ntc4

                    register = discord.Embed(title=f"{bot.user.name} 자가진단", description=reg_ntc, color=0xffdc16)
                    register.set_thumbnail(url=ImageDict["List_Write"])
                        ...
                        async def OK(self, button: discord.ui.Button, interaction: discord.Interaction):
                            try:
                                ...
                                if hcskr_result['code'] == 'SUCCESS':
                                    ...
                                    Register_Success = discord.Embed(title=f"{ctx.author}님의 자가진단 정보 입력이 완료되었습니다.", description="매일 오전 7시~7시 10분 사이에 자가진단이 진행됩니다.",color=0xffdc16)
                                    Register_Success.set_thumbnail(url=ImageDict["List_Done"])
                                    ...
                                    Register_Test_Fail.set_thumbnail(url=ImageDict["List_Failed"])
                                ...
                                Failed_reg.set_thumbnail(url=ImageDict["List_Failed"])
    ...
    if 작업 == "예약":
        if TrueFalse:
            ...
            if json_data[UserID][0]['Auto_check'] == "O":
                ...
                Auto_off.set_thumbnail(url=ImageDict["Timer_Off"])
                ...
                Auto_on.set_thumbnail(url=ImageDict["Timer_On"])
